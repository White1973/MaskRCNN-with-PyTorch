...
def get_args():
    ...
    parser.add_argument('--model', default='fasterrcnn_resnet50_fpn', choices=['fasterrcnn_resnet50_fpn', 'maskrcnn_resnet50_fpn'], help='model')
    parser.add_argument('--dataset', default='coco', help='model')
    parser.add_argument('--score', type=float, default=0.95, help='objectness score threshold')
...
def main():
    ...
    for idx in range(boxes.shape[0]):
        if scores[idx] >= args.score:
            ...
            person_img = src_img[int(y1.item()):int(y2.item()), int(x1.item()):int(x2.item())]
            cv2.imwrite('./results/{}'.format(args.image_path.split('/')[-1]), person_img)
